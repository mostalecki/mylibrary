...
class AddTvShow extends StatefulWidget {
  ...
  AddTvShow({this.snackKey, this.editMode, this.movie});
...
class _AddTvShowState extends State<AddTvShow> {
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      title: widget.editMode ? Text('Edit tv show') : Text('Add tv show'),
      content: Form(
        key: _formKey,
        child: Column(
          children: <Widget>[
      ...
      actions: <Widget>[
        MaterialButton(
            ...
            onPressed: () async {
              if (widget.editMode){
                await _db.deleteMovie(widget.movie, series: true);
                Navigator.pop(context);
                widget.snackKey.currentState.showSnackBar(
                    SnackBar(
                      content: Text('Tv show deleted'),
                    )
                );
              }
            }
          ...
          onPressed: () async {
            if (_formKey.currentState.validate()){
              ...
              if (!widget.editMode) {
                ...
                widget.snackKey.currentState.showSnackBar(
                    SnackBar(
                      content: Text('Tv show added'),
                    )
                );
              } else{
                movie.key = widget.movie.key;
                await _db.editMovie(movie, series: true);
                Navigator.pop(context);
                widget.snackKey.currentState.showSnackBar(
                    SnackBar(
                      content: Text('Tv show edited'),
                    )
                );
              }
            }
